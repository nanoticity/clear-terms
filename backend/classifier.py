...
def analyze_tos(tos_text: str) -> dict:
    ...
    avg_confidences = {
        k: round(sum(v) / len(v), 3) if v else 0 for k, v in confidences.items()
    }
    ...
    return {
        ...
        "sample_clauses": {
            "bad": [
                c for c, p in zip(clauses, predictions) if p["classification"] == "bad"
            ][:2],
            "blocker": [
                c
                for c, p in zip(clauses, predictions)
